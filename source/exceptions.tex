...
shall appear only within a \grammarterm{lambda-declarator}
...
on the top-level declarator, on the \grammarterm{declarator} of one of its
\grammarterm{parameter-declaration}{s} (if any), or on its return type.
\enterexample
\begin{codeblock}
void f() throw(int);                    // OK
...
variable with static or thread storage duration~(\ref{basic.start.static})
