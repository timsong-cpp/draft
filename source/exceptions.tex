...
the destructor is invoked for all each of its the object's fully constructed
...
\begin{itemize}
\item
If the declaration of \tcode{f} has a non-throwing
\grammarterm{exception-specification}, the set is empty.
\item
Otherwise, if the declaration of \tcode{f} has a
\grammarterm{dynamic-exception-specification}, the set consists of
every type in that \grammarterm{dynamic-exception-specification}.
\item
...
\item
if \tcode{e} is a \grammarterm{throw-expression}~(\ref{except.throw}),
...
\pnum
Given a member function \tcode{f} of some class \tcode{X}, where \tcode{f} is
an inheriting constructor~(\ref{class.inhctor}) or an implicitly-declared special
member function,
the \defn{set of potential exceptions of the implicitly-declared member function} \tcode{f}
...
when an exception is thrown during the execution of a constructor (\ref{except.ctor}), their \grammarterm{exception-specification}s do not contribute to the
\grammarterm{exception-specification} of the constructor, because
...
implicit \grammarterm{exception-specification}, as follows, where \tcode{f} is
the member function and \placeholder{S} is the set of potential exceptions of the
implicitly-declared member function \tcode{f}:
\begin{itemize}
\item
if \placeholder{S} contains the pseudo-type “any”, the implicit
\grammarterm{exception-specification} is \tcode{noexcept(false)};
\item
otherwise, if \placeholder{S} contains at least one type, the implicit
\grammarterm{exception-specification} specifies each type \tcode{T}
contained in \placeholder{S};
\item
...
struct D : public A, public B {
    ...
    // Implicit declaration of \tcode{D::D() throw(X, std::bad_array_new_length)};
