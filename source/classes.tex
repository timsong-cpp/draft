...
\pnum
\DIFdelbegin %DIFDELCMD < \indextext{definition!empty class}%%%
%DIF < 
\DIFdelend Complete objects and member subobjects of class type shall have nonzero
size.\footnote{Base class subobjects are not so constrained.}
\DIFdelbegin %DIFDELCMD < \indextext{\idxcode{sizeof}!empty class}%%%
%DIF < 
\DIFdelend \enternote
...
A \term{trivial class} is a class that has a trivial default constructor~(\ref{class.ctor}) and is trivially copyable.
...
\pnum
A member can be initialized using a \grammarterm{brace-or-equal-initializer}. (For static data members, see~\ref{class.static.data}; for non-static data members, see~\ref{class.base.init}).
...
\grammarterm{brace-or-equal-initializer}.
...
\pnum
A \grammarterm{union-like class} is a union or a class that has an anonymous union as a direct member. A union-like class \tcode{X} has a set of \term{variant members}. If \tcode{X} is a union its variant members are the non-static data members; otherwise, its variant members are the non-static data members of all anonymous unions that are members of \tcode{X}.
