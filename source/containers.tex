...
\tcode{X::value_type}       &
 \tcode{T}                  &
                            &
 \requires\ \tcode{T} is \tcode{Destructible} &
 compile time               \\ \rowsep
...
argument~(\ref{allocator.requirements}), an allocator whose value type is the
...
\tcode{X::const_reverse_iterator}         &
 iterator type whose value type is \tcode{const T}  &
...
an expression \tcode{v} of type \tcode{T}, and
...
post: \tcode{u == t}, \tcode{get_allocator() == m} &
linear													\\ \rowsep
...
\tcode{X u(rv)}
  ...
  construction; the value of \tcode{get_allocator()} shall be the same as the
...
\tcode{X u(rv, m);}			&
  ...
  this construction, \tcode{get_allocator() == m}												&
  constant if \tcode{m ==} \tcode{rv.get_allocator()}, otherwise linear	\\ \rowsep

\tcode{a = t}             &
  \tcode{X\&}             &
  \requires\ \tcode{T} is
  \DIFdelbegin \DIFdel{CopyInsertable }\DIFdelend \DIFaddbegin \tcode{CopyInsertable} \DIFaddend into \tcode{X}
...
sequence, excepting \tcode{vector<bool>}, are modified concurrently.
...
\tcode{X a(n, t)}   &
 ...
 \DIFdelbegin \DIFdel{CopyInsertable }\DIFdelend \DIFaddbegin \tcode{CopyInsertable} \DIFaddend into \tcode{X}.\br
...
\tcode{a.insert(p,t)}   &
 \tcode{iterator}       &
 \requires \tcode{T} shall be
 \DIFdelbegin \DIFdel{CopyInsertable }\DIFdelend \DIFaddbegin \tcode{CopyInsertable} \DIFaddend into \tcode{X}. For \tcode{vector} and \tcode{deque},
...
\tcode{a.clear()}   &
 ...
 post: \tcode{a.empty()} returns \tcode{true}      \\ \rowsep
...
\tcode{X::key_type}     &
 \tcode{Key}            &
 \requires\ \tcode{Key} is \tcode{Destructible}.  &
  compile time \\ \rowsep
...
\tcode{a = il}     &
  \tcode{X\&}               &
  \requires\ \DIFdelbegin %DIFDELCMD < \tcode{T} %%%
\DIFdel{is
  CopyInsertable }\DIFdelend \DIFaddbegin \tcode{value_type} \DIFadd{is
  }\tcode{CopyInsertable} \DIFaddend into \tcode{X}
  and \tcode{CopyAssignable}.\br
...
\tcode{a_uniq.} \tcode{emplace(args)} &
 \tcode{pair<iterator,} \tcode{bool>} &
  \requires\ \DIFdelbegin %DIFDELCMD < \tcode{T} %%%
\DIFdelend \DIFaddbegin \tcode{value_type} \DIFaddend shall be \tcode{EmplaceConstructible} into \tcode{X} from \tcode{args}.\br
  \effects\ Inserts a \DIFdelbegin %DIFDELCMD < \tcode{T} %%%
\DIFdelend \DIFaddbegin \tcode{value_type} \DIFaddend object \tcode{t} constructed with
...
\tcode{a_uniq.} \tcode{insert(t)}      &
  ...
  \requires\ If \tcode{t} is a non-const rvalue expression, \DIFdelbegin %DIFDELCMD < \tcode{T} %%%
\DIFdelend \DIFaddbegin \tcode{value_type} \DIFaddend shall be
  \tcode{MoveInsertable} into \tcode{X}; otherwise, \DIFdelbegin %DIFDELCMD < \tcode{T} %%%
\DIFdel{shall be
  CopyInsertable }\DIFdelend \DIFaddbegin \tcode{value_type} \DIFadd{shall be
  }\tcode{CopyInsertable} \DIFaddend into \tcode{X}.\br
...
\tcode{a.insert(}\br
  ...
  \tcode{void}                   &
  \requires\ \DIFdelbegin %DIFDELCMD < \tcode{T} %%%
\DIFdelend \DIFaddbegin \tcode{value_type} \DIFaddend shall be \tcode{EmplaceConstructible} into \tcode{X} from \tcode{*i}.\br
...
\tcode{a.erase(}\br
 \tcode{q1, q2)}  &
 \tcode{iterator}        &
 erases all the elements in the range \range{q1}{q2}. Returns \tcode{q2}.  &
...
\tcode{X::key_type}   &
  ...
  \requires\ \tcode{Key} shall be \tcode{Destructible}.  &
  compile time        \\ \rowsep
...
&   \tcode{X\&}
&   \requires\ \DIFdelbegin %DIFDELCMD < \tcode{T} %%%
\DIFdelend \DIFaddbegin \tcode{value_type} \DIFaddend is
...
such that \tcode{distance(Ea1, Ea2) == distance(Eb1, Eb2)} and
\tcode{is_permutation(Ea1, Ea2, Eb1)} returns \tcode{true}. For
...
%DIFDELCMD <   %%%
\DIFdelend template <class T, size_t N > struct array;
...
\begin{codeblock}
\DIFdelbegin \DIFdel{namespace std }%DIFDELCMD < {
%DIFDELCMD <   %%%
\DIFdelend #include <initializer_list>

\DIFaddbegin \DIFadd{namespace std }{
  \DIFaddend template <class T, class Allocator = allocator<T> > class deque;
...
\DIFaddbegin \DIFadd{namespace std }{
  \DIFaddend template <class T, class Allocator = allocator<T> > class forward_list;
  template <class T, class Allocator>
...
\DIFaddbegin \DIFadd{namespace std }{
  \DIFaddend template <class T, class Allocator = allocator<T> > class list;
  template <class T, class Allocator>
...
%DIFDELCMD <   %%%
\DIFdelend template <class T, class Allocator = allocator<T> > class vector;
  template <class T, class Allocator>
...
namespace std {
  ...
  struct array {
    ...
    void fill(const T& u);
    void swap(array\DIFdelbegin \DIFdel{<T, N>}\DIFdelend &) noexcept(noexcept(swap(declval<T&>(), declval<T&>())));
...
\pnum
\realnote Unlike the \tcode{swap} function for other containers, array::swap
...
namespace std {
  ...
  public:
    ...
    deque(const deque\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x);
    deque(deque&&);
    ...
    ~deque();
    deque\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & operator=(const deque\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x);
    deque\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & operator=(deque\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x);
    ...
    void     swap(deque\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend &);
    void     clear() noexcept;
  };
...
\tcode{n} value-initialized elements.

\pnum
\requires \tcode{T} shall be \tcode{DefaultConstructible}.
...
of interest, but in a \tcode{forward_list} there is no constant-time way to acess a
...
namespace std {
  ...
  public:
    ...
    ~forward_list();
    forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & operator=(const forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x);
    forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & operator=(forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x);
    ...
    iterator erase_after(const_iterator position, \DIFdelbegin \DIFdel{iterator }\DIFdelend \DIFaddbegin \DIFadd{const_iterator }\DIFaddend last);
    void swap(forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend &);
    ...
    void splice_after(const_iterator position, forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x);
    void splice_after(const_iterator position, forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x);
    void splice_after(const_iterator position, forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x,
                      const_iterator i);
    void splice_after(const_iterator position, forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x,
    ...
    void merge(forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x);
    void merge(forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x);
    template <class Compare> void merge(forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x, Compare comp);
    template <class Compare> void merge(forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x, Compare comp);
...
\begin{itemdescr}
\pnum
\effects Constructs a \tcode{forward_list} object with \tcode{n} value-initialized elements.
...
\indexlibrary{\idxcode{assign}!\idxcode{forward_list}}%
\indexlibrary{\idxcode{forward_list}!\idxcode{assign}}%
\begin{itemdecl}
template <class InputIterator>
  void assign(InputIterator first, InputIterator last);
\end{itemdecl}
...
elements at the end of the list. For the first signature the inserted elements are
value-initialized, and for the second signature they are
copies of \tcode{c}.

\pnum
\requires \tcode{T} shall be \tcode{DefaultConstructible} for the first form and it shall be
\tcode{CopyInsertable} into \tcode{*this} for the second form.
\end{itemdescr}
...
\indexlibrary{\idxcode{forward_list}!\idxcode{splice_after}}%
\begin{itemdecl}
void splice_after(const_iterator position, forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x, const_iterator i);
void splice_after(const_iterator position, forward_list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x, const_iterator i);
\end{itemdecl}
...
\tcode{*this}. Iterators to \tcode{*i} (including \tcode{i} itself) continue to refer to
...
\tcode{pred(*i)} is true (for \tcode{remove_if()}). This operation shall be stable: the
...
\pnum
\effects Merges \tcode{x} into \tcode{*this}. This operation shall be stable: for equivalent
elements in the two lists, the elements from \tcode{*this} shall always precede the elements
from \tcode{x}. \tcode{x} is empty after the merge. If an exception is thrown other than by a
...
an exception is thrown the order of the elements in \tcode{*this} is unspecified.
...
namespace std {
  ...
  public:
    ...
    list(const list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x);
    list(list&& x);
    list(const list&, const Allocator&);
    list(list&&, const Allocator&);
    list(initializer_list<T>, const Allocator& = Allocator());
   ~list();
    list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & operator=(const list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x);
    list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & operator=(list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x);
    ...
    void     swap(list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend &);
    void     clear() noexcept;

    // \ref{list.ops}, list operations:
    void splice(const_iterator position, list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x);
    void splice(const_iterator position, list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x);
    void splice(const_iterator position, list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x, const_iterator i);
    void splice(const_iterator position, list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x, const_iterator i);
    void splice(const_iterator position, list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x,
                const_iterator first, const_iterator last);
    void splice(const_iterator position, list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x,
    ...
    void merge(list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x);
    void merge(list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x);
    template <class Compare> void merge(list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x, Compare comp);
    template <class Compare> void merge(list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x, Compare comp);
...
\pnum
\effects
Replaces the contents of the list with \tcode{n} copies of \tcode{t}.
\end{itemdescr}
...
\indexlibrary{\idxcode{list}!\idxcode{splice}}%
\begin{itemdecl}
void splice(const_iterator position, list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x, const_iterator first,
            const_iterator last);
void splice(const_iterator position, list\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x, const_iterator first,
            const_iterator last);
\end{itemdecl}
  ...
  public:
    ...
    template <class InputIterator>
      vector(InputIterator first, InputIterator last,
             const Allocator& = Allocator());
    vector(const vector\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x);
    vector(vector&&);
    vector(const vector&, const Allocator&);
    vector(vector&&, const Allocator&);
    vector(initializer_list<T>, const Allocator& = Allocator());
   ~vector();
    vector\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & operator=(const vector\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x);
    vector\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & operator=(vector\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend && x);
    ...
    iterator erase(const_iterator position);
    iterator erase(const_iterator first, const_iterator last);
    void     swap(vector\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend &);
    void     clear() noexcept;
  };
...
\begin{itemdecl}
void swap(vector\DIFdelbegin \DIFdel{<T,Allocator>}\DIFdelend & x);
\end{itemdecl}
...
there are no effects.
If an exception is thrown by the move constructor of a non-\tcode{CopyInsertable}
\tcode{T}, the effects are unspecified.
...
\begin{itemdescr}
\pnum
\requires If the iterator's dereference operator returns an lvalue or a
...
\tcode{CopyConstructible} and \tcode{mapped_type} shall be
\tcode{DefaultConstructible}.
...
\pnum
\requires
\tcode{P} shall be convertible to \tcode{value_type}.
...
\tcode{iterator}~(\ref{associative.reqmts}).
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\requires \tcode{mapped_type} shall be \tcode{DefaultConstructible}.
For the first operator, \tcode{key_type} shall be \tcode{CopyConstructible}.
...
\DIFdelbegin %DIFDELCMD < \rSec3[unord.map.modifers]{\tcode{unordered_map} modifiers}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \rSec3[unord.map.modifiers]{\tcode{unordered_map} modifiers}
\DIFaddend 
...
\begin{itemdescr}
\pnum
\requires \tcode{value_type} is constructible from \tcode{std::forward<P>(obj)}.

\pnum
\effects Inserts \tcode{obj} converted to \tcode{value_type} if and only if there is no element
in the container with key equivalent to the key of \tcode{value_type(obj)}.

\pnum
\returns The \tcode{bool} component of the returned \tcode{pair} object indicates whether
the insertion took place and the iterator component points to the element with key equivalent
to the key of \tcode{value_type(obj)}.

\pnum
\complexity Average case \bigoh{1}, worst case \bigoh{\tcode{size()}}.

\pnum
\remarks This signature shall not participate in overload resolution unless \tcode{P}
is implicitly convertible to \tcode{value_type}.
\end{itemdescr}
...
\effects Inserts \tcode{obj} converted to \tcode{value_type} if and only if there is no element
in the container with key equivalent to the key of \tcode{value_type(obj)}.
The iterator \tcode{hint} is a hint pointing to where the search should start.

\pnum
\returns An iterator that points to the element with key equivalent
to the key of \tcode{value_type(obj)}.
...
\DIFdelbegin %DIFDELCMD < \rSec3[unord.multimap.modifers]{\tcode{unordered_multimap} modifiers}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \rSec3[unord.multimap.modifiers]{\tcode{unordered_multimap} modifiers}
\DIFaddend 

\indexlibrary{\idxcode{unordered_multimap}!\idxcode{operator[]}}%
\indexlibrary{\idxcode{operator[]}!\idxcode{unordered_multimap}}%
\begin{itemdecl}
template <class P>
...
\pnum
\effects Inserts \tcode{obj} converted to \tcode{value_type}.
...
\tcode{queue}, \tcode{priority_queue}, and \tcode{stack}. These container
adaptors meet the requirements for sequence containers.
