...
\tcode{X::value_type}       &
 \tcode{T}                  &
                            &
 \removed{\requires}\added{\expects}\ \tcode{T} is \oldconcept{Erasable} from \tcode{X} (see~\ref{container.requirements.general}, below) &
 compile time               \\ \rowsep
...
\tcode{X(a)}                &
                            &
                            &
 \removed{\requires}\added{\expects} \tcode{T} is \oldconcept{CopyInsertable}
...
\tcode{a == b}                  &
 ...
 \removed{\requires}\added{\expects}\ \tcode{T} is \oldconcept{\-Equal\-ity\-Compar\-a\-ble} &
...
\tcode{a < b}                   &
 ...
 \removed{\requires}\added{\expects} \tcode{<} is defined for values of \tcode{T}. \tcode{<} is a total ordering relationship.    &
 linear                     \\ \rowsep
...
\tcode{X u;}							&
													&
  \removed{\requires}\added{\expects}\ \tcode{A} is \oldconcept{DefaultConstructible}.\br
...
\tcode{X u(t, m);}				&
                          &
\removed{\requires}\added{\expects}\ \tcode{T} is \oldconcept{CopyInsertable} into \tcode{X}.\br
...
\tcode{a = rv}          &
  \tcode{X\&}           &
  \removed{\requires}\added{\expects}\ If \tcode{allocator_-}\br
...
\tcode{X u(n, t);}   &
                &
 \removed{\requires}\added{\expects}\ \tcode{T} shall be
...
\tcode{X u(i, j);}   &
                    &
 \removed{\requires}\added{\expects}\ \tcode{T} shall be \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{*i}.
...
\tcode{a.emplace(p, args)}  &
 \tcode{iterator}            &
 \removed{\requires}\added{\expects}\ \tcode{T} is \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{args}. For \tcode{vector} and \tcode{deque},
 \tcode{T} is also
...
\tcode{a.insert(p,i,j)}    &
 ...
 Each iterator in the range \range{i}{j} shall be dereferenced exactly once.\br
 \removed{\requires}\added{\expects} \tcode{i} and \tcode{j} are not iterators into \tcode{a}.\br
...
\tcode{a.erase(q)}  &
 \tcode{iterator}   &
 \removed{\requires}\added{\expects}\ For \tcode{vector} and \tcode{deque},
...
\tcode{a.assign(i,j)}   &
 ...
 \oldconcept{MoveInsertable} into \tcode{X}.\br
 Each iterator in the range \range{i}{j} shall be dereferenced exactly once.\br
 \removed{\requires}\added{\expects} \tcode{i}, \tcode{j} are not iterators into \tcode{a}.\br
...
\tcode{a.assign(n,t)}   &
 ...
 \oldconcept{CopyInsertable} into \tcode{X}
 and \oldconcept{CopyAssignable}.\br
 \removed{\requires}\added{\expects} \tcode{t} is not a reference into \tcode{a}.\br
...
\tcode{a.emplace_\-front(args)}      &
 ...
 \removed{\requires}\added{\expects}\ \tcode{T} shall be \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{args}.\br
...
\tcode{a.emplace_\-back(args)}      &
 ...
 \removed{\requires}\added{\expects}\ \tcode{T} shall be \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{args}. For \tcode{vector}, \tcode{T}
 shall also be
 \oldconcept{MoveInsertable} into \tcode{X}.\br
 \returns{} \tcode{a.back()}.  &
 \tcode{deque},
...
\tcode{a.pop_front()}      &
 ...
 \removed{\requires}\added{\expects}\ \tcode{a.empty()} shall be \tcode{false}. &
 \tcode{deque},
 \tcode{forward_list},
 \tcode{list}
 \\ \rowsep
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} Either \tcode{!alloc_}, or
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \tcode{empty() == false}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \tcode{!alloc_}, or \tcode{!nh.alloc_}, or
...
\tcode{X::value_type} (\tcode{set} and \tcode{multiset} only) &
 \tcode{Key}            &
  \removed{\requires}\added{\expects}\ \tcode{value_type} is \oldconcept{Erasable} from \tcode{X} &
  compile time \\ \rowsep
...
\tcode{X::key_compare}  &
  \tcode{Compare}       &
  \removed{\requires}\added{\expects}\ \tcode{key_compare} is \oldconcept{CopyConstructible}. &
  compile time   \\ \rowsep
...
\tcode{X()}\br\tcode{X u;}                      &
                                                &
  \removed{\requires}\added{\expects}\ \tcode{key_compare} is \oldconcept{DefaultConstructible}.\br
  \effects\ Constructs an empty container.
...
\tcode{X~u(i,j,c);}     &
                        &
  \removed{\requires}\added{\expects}\ \tcode{value_type} is \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{*i}.\br
...
\tcode{X(i,j)}\br\tcode{X~u(i,j);}    &
                                    &
  \removed{\requires}\added{\expects}\ \tcode{key_compare} is \oldconcept{DefaultConstructible}.
...
\tcode{a = il}     &
  \tcode{X\&}               &
  \removed{\requires}\added{\expects}\ \tcode{value_type} is
  \oldconcept{CopyInsertable} into \tcode{X}
  and \oldconcept{CopyAssignable}.\br
...
\tcode{a_uniq.\brk{}emplace(\brk{}args)} &
 \tcode{pair<\brk{}iterator, bool>} &
  \removed{\requires}\added{\expects}\ \tcode{value_type} shall be \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{args}.\br
...
\tcode{a_uniq.\brk{}insert(\brk{}t)}      &
  \tcode{pair<\brk{}iterator, bool>}   &
  \removed{\requires}\added{\expects}\ If \tcode{t} is a non-const rvalue, \tcode{value_type} shall be
...
\tcode{a.\brk{}insert(\brk{}i, j)}          &
  \tcode{void}                   &
  \removed{\requires}\added{\expects}\ \tcode{value_type} shall be \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{*i}.\br
  \removed{\requires}\added{\expects} \tcode{i}, \tcode{j} are not iterators into \tcode{a}.
...
\tcode{a_uniq.\brk{}insert(\brk{}nh)}           &
 ...
 \removed{\requires}\added{\expects} \tcode{nh} is empty or
...
\tcode{a.merge(a2)}              &
 \tcode{void}             &
 \removed{\requires}\added{\expects} \tcode{a.get_allocator() == a2.get_allocator()}.\br
...
&   \tcode{Hash::transparent_key_equal} if such a \grammarterm{qualified-id}
    is valid and denotes a type\iref{temp.deduct};
    otherwise, \tcode{Pred}.
&   \removed{\requires}\added{\expects}\ \tcode{Pred} is \oldconcept{CopyConstructible}.\br
...
&   \tcode{X}
&   \removed{\requires}\added{\expects}\ \tcode{key_equal} is \oldconcept{DefaultConstructible}.\br
    \effects\ Constructs an empty container with at least \tcode{n} buckets,
...
&   \tcode{X}
&   \removed{\requires}\added{\expects}\ \tcode{hasher} and \tcode{key_equal} are \oldconcept{DefaultConstructible}.\br
    \effects\ Constructs an empty container with at least \tcode{n} buckets,
...
&   \tcode{X}
&   \removed{\requires}\added{\expects}\ \tcode{key_equal} is \oldconcept{DefaultConstructible}.
    \tcode{value_type} is \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{*i}.\br
    \effects\ Constructs an empty container with at least \tcode{n} buckets,
using \tcode{hf} as the hash function and \tcode{key_equal()} as the key
equality predicate, and inserts elements from \tcode{[i, j)} into it.
...
&   \tcode{X}
&   \removed{\requires}\added{\expects}\ \tcode{hasher} and \tcode{key_equal} are \oldconcept{DefaultConstructible}.
    \tcode{value_type} is \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{*i}.\br
    \effects\ Constructs an empty container with at least \tcode{n} buckets,
using \tcode{hasher()} as the hash function and \tcode{key_equal()}
...
&   \tcode{void}
&   \removed{\requires}\added{\expects}\ \tcode{value_type} shall be \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{*i}.\br
    \removed{\requires}\added{\expects} \tcode{i} and \tcode{j} are not iterators in \tcode{a}.\br
...
& \tcode{size_type}
&
  \removed{\requires}\added{\expects} \tcode{b.bucket_count() > 0}.\br
...
&   \tcode{size_type}
&   \removed{\requires}\added{\expects} \tcode{n} shall be in the range \tcode{[0, b.bucket_count())}.
...
&   \tcode{void}
&   \removed{\requires}\added{\expects} \tcode{z} shall be positive.
...
\pnum
\removed{\requires}\added{\expects} \tcode{T} shall be \oldconcept{DefaultInsertable} into \tcode{*this}.
...
\pnum
\removed{\requires}\added{\expects} \tcode{T} shall be \oldconcept{CopyInsertable} into \tcode{*this}.
...
\pnum
\removed{\requires}\added{\expects} \tcode{T} shall be \oldconcept{MoveInsertable} and \oldconcept{DefaultInsertable} into \tcode{*this}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \tcode{T} shall be \oldconcept{MoveInsertable} into \tcode{*this}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \tcode{position} is \tcode{before_begin()} or is a dereferenceable
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The iterator following \tcode{position} is dereferenceable.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} All iterators in the range \orange{position}{last} are dereferenceable.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \tcode{position} is \tcode{before_begin()} or is a
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \tcode{*this} and \tcode{x} are both sorted with respect to
...
\added{
\pnum
\constraints \tcode{is_swappable_v<Container>} is \tcode{true}.
}
...
\added{
\pnum
\constraints \tcode{is_swappable_v<Container>} is \tcode{true} and
\tcode{is_swappable_v<Compare>} is \tcode{true}.
}
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \range{ptr}{ptr + count} shall be a valid range.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\requires}
