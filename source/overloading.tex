...
For direct-initialization or default-initialization, the
...
\pnum
If the parameter type is a class \tcode{X} and the initializer list has a
single element of type \cvqual{cv} \tcode{U}, where \tcode{U} is \tcode{X}
...
if the initializer list has exactly \tcode{N} elements or if it has fewer
...
the parameter type; \enterexample
\begin{codeblock}
void f(int);
