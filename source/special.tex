...
\pnum
\indextext{object~temporary|see{temporary object}}%
\indextext{temporary!object}%
...
Temporary objects of class or array type are created in various contexts:
...
and in some initializations~(\ref{dcl.init}).
...
\grammarterm{decltype-specifier}~(\ref{expr.call}), no temporary object is introduced,
...
Similarly, the destructor shall be called for a temporary object with a non-trivial
...
the destruction of every temporary object created in
...
does not bind directly~(\ref{dcl.init.ref}) or
is initialized with a temporary expression.
...
The corresponding temporary object (if any)
persists for the lifetime of the reference except:

\begin{itemize}
\item A temporary object bound to a reference member in a constructor's \grammarterm{ctor-initializer}~(\ref{class.base.init}) persists until the constructor exits.

\item A temporary object bound to a reference parameter in a function call~(\ref{expr.call})
persists until the completion of the full-expression containing the call.

\item The lifetime of a temporary object bound to the returned value in a function return statement~(\ref{stmt.return}) is not extended; the temporary is destroyed at the end of the full-expression in the return statement.

\item A temporary object bound to a reference in a \grammarterm{new-initializer}~(\ref{expr.new}) persists until the completion of the full-expression containing the \grammarterm{new-initializer}. \enterexample
\begin{codeblock}
...
The destruction of a temporary object whose lifetime is not extended by being
bound to a reference is sequenced before the destruction of every
temporary object which is constructed earlier in the same full-expression.
If the lifetime of two or more temporary objects to which references are bound ends
at the same point,
these temporary objects are destroyed at that point in the reverse order of the
completion of their construction.
In addition, the destruction of temporary objects bound to references shall
...
that is, if
\tcode{obj1}
is an object with the same storage duration as a temporary object \tcode{t} and is
created before \tcode{t} is created,
\tcode{t} shall be destroyed before
\tcode{obj1}
is destroyed;
if
\tcode{obj2}
is an object with the same storage duration as \tcode{t} and is
created after \tcode{t} is created,
\tcode{t} shall be destroyed after
\tcode{obj2}
is destroyed.
\enterexample
...
\pnum
When an object of class type \placeholder{X}
is passed to or returned from a function,
if \placeholder{X} has a trivial, accessible copy or move constructor that is not deleted,
and \placeholder{X} has no non-trivial copy constructors, move constructors, or destructors,
implementations are permitted to perform an additional copy or move of the object
using the trivial constructor
(even if it would not be selected by overload resolution to perform a copy or move of the object).
\enternote
This latitude is granted to allow objects of class type to be
...
\begin{itemize}
\item for each non-template constructor of \tcode{X},
...
\item for each constructor template of \tcode{X},
...
\item absence or presence of \tcode{explicit}~(\ref{class.conv.ctor}).
\end{itemize}
...
in \tcode{X}.
It is \tcode{constexpr} if the user-written constructor (see below)
would satisfy the requirements of a
\tcode{constexpr} constructor~(\ref{dcl.constexpr}).
It is deleted if the corresponding constructor in \tcode{X} is
...
class \tcode{X} is not considered in the determination.
When performing template argument substitution~(\ref{temp.deduct})
for a constructor template so declared,
name lookup, overload resolution, and access checking
are performed in the context of
the corresponding constructor template of \tcode{X}.
\enterexample
\begin{codeblock}
struct B {
...
B b(S(), 1); // OK: \tcode{B::B} is a friend of \tcode{S}
D d(S(), 2); // OK: access control is in the context of \tcode{B::B}
\end{codeblock}
\exitexample
