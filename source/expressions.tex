...
scope~(\ref{basic.scope.local}) is a \defn{local lambda expression}; any other
...
reference-compatible with ``\cvqual{cv1} \tcode{T1}''~(\ref{dcl.init.ref}). The
result refers to the object or the specified base class subobject thereof. If
\tcode{T2} is an inaccessible (Clause~\ref{class.access}) or
ambiguous~(\ref{class.member.lookup}) base class of \tcode{T1},
a program that necessitates such a cast is ill-formed.

\pnum
Otherwise, an expression \tcode{e} can be explicitly converted to a type
...
\tcode{reinterpret_cast<T\&\&>(x)}). The result refers to the same
...
\grammarterm{noptr-new-declarator} shall be an integral constant
expression~(\ref{expr.const}) and evaluate to a strictly positive value.
\indextext{\idxcode{new}}%
The \grammarterm{expression} in a \grammarterm{noptr-new-declarator} shall be
...
type. The result has type \tcode{void}.\footnote{This implies that an object
...
contextually converted to type \tcode{bool}
(Clause~\ref{conv}). The
...
\begin{itemize}
\item
\tcode{this}~(\ref{expr.prim}) unless it appears as
the \grammarterm{postfix-expression} in a class member access
...
undefined \tcode{constexpr} constructor outside the definition of a \tcode{constexpr}
function or a \tcode{constexpr} constructor;
...
constexpr const int* xp = addr(x);  // OK: \tcode{(const int*)\&(const int\&)x} is an
                                    // address \DIFdelbegin \DIFdel{contant }\DIFdelend \DIFaddbegin \DIFadd{constant }\DIFaddend expression
...
\grammarterm{mem-initializer}{s}; \enterexample
\begin{codeblock}
int x;                              // not constant
struct A {
...
\begin{itemize}
\item
a glvalue of integral or enumeration type that refers to a non-volatile const
...
\item
a glvalue of literal type that refers to a non-volatile temporary object
...
\item
a subtraction~(\ref{expr.add}) where both operands are pointers;
...
\pnum
A \term{literal constant expression} is a prvalue core constant expression of
literal type, but not pointer type. An \term{integral constant expression} is a
...
\exitnote
A \term{converted constant expression} of type \tcode{T} is a literal constant
expression, implicitly converted to type \tcode{T}, where the implicit
...
A \term{reference constant expression} is an lvalue core constant expression
that designates an object with static storage duration or a function. An
\term{address constant expression} is a prvalue core constant expression of
...
core constant expression of type \tcode{std::nullptr_t}. Collectively, literal
constant expressions, reference constant expressions, and address constant
expressions are called \term{constant expressions}.
...
enumeration type 
and that conversion function shall be \tcode{constexpr}. \enterexample
\begin{codeblock}
struct A { 
