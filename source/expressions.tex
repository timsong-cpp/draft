...
\pnum
\indextext{temporary!expression}%
An expression may be a \term{temporary expression},
in which case the lifetime of its temporary object (if any)
may be extended~(\ref{class.temporary}).
...
\pnum
\indextext{closure object}%
A \grammarterm{lambda-expression} is a prvalue temporary expression.
Its value is a temporary object called the \defn{closure object}. A
...
\exitnote, except that in the case of an array operand:
\begin{itemize}
\item the result is an lvalue if that operand is an lvalue and an xvalue otherwise, and
\item the result is a temporary expression if that operand is a temporary expression.
\end{itemize}
...
\item otherwise, the type of the prvalue shall be complete and,
if the result type is a class type,
the function call is a temporary expression
whose value is a temporary object.
\end{itemize}
...
\grammarterm{typename-specifier}~(\ref{temp.res})
designating a type \placeholder{T}
followed by
a \grammarterm{braced-init-list} or
a parenthesized \grammarterm{expression-list}
constructs a value of type \placeholder{T} as follows:

\begin{itemize}
\item
In the case of an expression list with a
single expression \placeholder{e},
the type conversion expression is equivalent
(in definedness, and if defined in meaning)
to the corresponding cast expression
\tcode{(T)(e)}~(\ref{expr.cast}).

\item
The expression \tcode{T()},
where \tcode{T} is (possibly cv-qualified) \tcode{void},
creates a prvalue of \tcode{void} type.

\item
Otherwise, \placeholder{T} shall be
an object type or
a reference to such a type.
If \placeholder{T} is a class type or reference to class type,
the class type shall be complete.
The result of the conversion expression is
a value of type \placeholder{T}
direct-initialized~(\ref{dcl.init})
with the \grammarterm{braced-init-list}
or parenthesized \grammarterm{expression-list}
as the initializer.
If \placeholder{T} is an lvalue reference, the result is an lvalue;
if \placeholder{T} is an rvalue reference, the result is an xvalue;
otherwise, the result is a prvalue.
If \placeholder{T} is an array or class type or
a reference to such a type,
the result is a temporary object and
the conversion expression is a temporary expression.
\end{itemize}
...
member of the object designated by the first expression.
If \tcode{E1} is a temporary expression
and \tcode{E2} does not designate a bit-field,
then \tcode{E1.E2} is a temporary expression.
If \tcode{E1}
...
If \tcode{v} is a temporary expression
and \tcode{T} is a reference type,
...
otherwise, the result is a prvalue.
If \tcode{v} is a temporary expression
and \tcode{T} is ``reference to \placeholder{U}'',
where \placeholder{U} is reference-compatible with
the type of \tcode{v}~(\ref{dcl.init.ref}),
the result is a temporary expression.
The \tcode{static_cast} operator shall not
cast away constness~(\ref{expr.const.cast}).
...
the \tcode{static_cast} is a temporary expression
whose value is the temporary object \tcode{t}.
...
the result is a temporary expression if the operand is a temporary expression.
...
the set of potential exceptions of the
\grammarterm{expression}~(\ref{except.spec}) is empty, and \tcode{false}
otherwise.%
\indextext{expression!unary|)}
...
\exitexample 
If \tcode{E1} is an rvalue,
the program is ill-formed
if \tcode{E2}
is a pointer to member function with
...
\grammarterm{ref-qualifier} \tcode{\&\&}.
If \tcode{E1} is a temporary expression
and \tcode{E2} is a pointer to data member,
then \tcode{E1.*E2} is a temporary expression.
The result of \tcode{E1.*E2}
where \tcode{E2} is a pointer to data member
is an lvalue if \tcode{E1} is an lvalue
and an xvalue otherwise.
If \tcode{E2} is a pointer to member function,
the result is a prvalue.
If \tcode{E2} is the null pointer to member value~(\ref{conv.mem}),
the behavior is undefined.
...
is of the type and value category of the other operand.
The \grammarterm{conditional-expression}
is a temporary expression if that operand is a temporary expression and
...
\tcode{T2} by copy-initializing a temporary object of type \tcode{T2} from
\tcode{E1} and using that object as the converted operand.
...
the result is of that type and value category
and it is a bit-field if the second or the third operand is a bit-field,
or if both are bit-fields.
The \grammarterm{conditional-expression} is a temporary expression
if the second or third operands, or both, are temporary expressions.
...
temporary object of the result type, which is copy-initialized from either the
second operand or the third operand depending on the value of the first
operand; the \grammarterm{conditional-expression} is a prvalue temporary
expression.
...
the comma expression is a temporary expression.
