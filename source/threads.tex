...
specified in~\DIFdelbegin \DIFdel{(\ref{time})}\DIFdelend \DIFaddbegin \DIFadd{\ref{time}}\DIFaddend .
...
should \DIFdelbegin \DIFdel{, but might not, }\DIFdelend decrease the duration of the wait when the clock is adjusted forwards.
\exitnote
...
\begin{itemdescr}
\DIFaddbegin \pnum
\DIFaddend If \tcode{joinable()}, calls \tcode{std::terminate()}. Otherwise, has no effects.
...
namespace std {
  ...
  public:
    ...
    \DIFdelbegin \DIFdel{;
    }\DIFdelend template <class Clock, class Duration>
      bool try_lock_until(const chrono::time_point<Clock, Duration>& abs_time);
    \DIFdelbegin \DIFdel{;
    }\DIFdelend void unlock();
...
namespace std {
  ...
  public:
    ...
    \DIFdelbegin \DIFdel{;
    }\DIFdelend template <class Rep, class Period>
      unique_lock(mutex_type& m, const chrono::duration<Rep, Period>& rel_time);
    \DIFdelbegin \DIFdel{;
    }\DIFdelend ~unique_lock();
...
void g() {
  ...
  std::call_once(flag2, initializer\DIFaddbegin \DIFadd{()}\DIFaddend );
}
...
class information {
  ...
  void verifier();
public:
  void verify() { std::call_once(verified, \DIFdelbegin \DIFdel{verifier}\DIFdelend \DIFaddbegin &\DIFadd{information::verifier, *this}\DIFaddend ); }
};
\end{codeblock}
\exitexample
\end{itemdescr}
...
\begin{enumerate}
\item the release of the mutex \DIFdelbegin \DIFdel{, }\DIFdelend and entry into the waiting state;
...
\begin{itemdescr}
\pnum\effects If any threads are blocked waiting for \tcode{*this}, unblocks one of those \DIFdelbegin \DIFdel{theads}\DIFdelend \DIFaddbegin \DIFadd{threads}\DIFaddend .
\end{itemdescr}
...
\DIFdelbegin %DIFDELCMD < \indexlibrary{\idxcode{condition}!\idxcode{wait}}%%%
\DIFdelend \DIFaddbegin \indexlibrary{\idxcode{condition_variable}!\idxcode{wait}}\DIFaddend %
\DIFdelbegin %DIFDELCMD < \indexlibrary{\idxcode{wait}!\idxcode{condition}}%%%
\DIFdelend \DIFaddbegin \indexlibrary{\idxcode{wait}!\idxcode{condition_variable}}\DIFaddend %
\begin{itemdecl}
template <class Predicate>
  void wait(unique_lock<mutex>& lock, Predicate pred);
\end{itemdecl}
...
\DIFdelbegin %DIFDELCMD < \indexlibrary{\idxcode{condition}!\idxcode{wait_for}}%%%
\DIFdelend \DIFaddbegin \indexlibrary{\idxcode{condition_variable}!\idxcode{wait_for}}\DIFaddend %
\DIFdelbegin %DIFDELCMD < \indexlibrary{\idxcode{wait_for}!\idxcode{condition}}%%%
\DIFdelend \DIFaddbegin \indexlibrary{\idxcode{wait_for}!\idxcode{condition_variable}}\DIFaddend %
\begin{itemdecl}
template <class Rep, class Period>
  cv_status wait_for(unique_lock<mutex>& lock,
                     const chrono::duration<Rep, Period>& rel_time);
...
\DIFdelbegin %DIFDELCMD < \indexlibrary{\idxcode{condition}!\idxcode{wait_until}}%%%
\DIFdelend \DIFaddbegin \indexlibrary{\idxcode{condition_variable}!\idxcode{wait_until}}\DIFaddend %
\DIFdelbegin %DIFDELCMD < \indexlibrary{\idxcode{wait_until}!\idxcode{condition}}%%%
\DIFdelend \DIFaddbegin \indexlibrary{\idxcode{wait_until}!\idxcode{condition_variable}}\DIFaddend %
\begin{itemdecl}
template <class Clock, class Duration, class Predicate>
  bool wait_until(unique_lock<mutex>& lock,
                  const chrono::time_point<Clock, Duration>& abs_time,
...
\pnum
\enternote The returned value indicates whether the predicate \DIFdelbegin \DIFdel{evaluates }\DIFdelend \DIFaddbegin \DIFadd{evaluated }\DIFaddend to
...
\DIFdelbegin %DIFDELCMD < \indexlibrary{\idxcode{condition}!\idxcode{wait}}%%%
\DIFdelend \DIFaddbegin \indexlibrary{\idxcode{condition_variable_any}!\idxcode{wait}}\DIFaddend %
\DIFdelbegin %DIFDELCMD < \indexlibrary{\idxcode{wait}!\idxcode{condition}}%%%
\DIFdelend \DIFaddbegin \indexlibrary{\idxcode{wait}!\idxcode{condition_variable_any}}\DIFaddend %
\begin{itemdecl}
template <class Lock, class Predicate>
  void wait(Lock& lock, Predicate pred);
\end{itemdecl}
...
\DIFdelbegin %DIFDELCMD < \postcondition
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \pnum
\postconditions
...
\pnum
The \DIFdelbegin \DIFdel{template function }\DIFdelend \DIFaddbegin \DIFadd{function template }\DIFaddend \tcode{async} provides a mechanism to launch a function potentially
...
\pnum
\returns An object of type \DIFdelbegin %DIFDELCMD < \tcode{future<typename result_of<F(Args...)>:type>} %%%
\DIFdelend \DIFaddbegin \tcode{future<typename result_of<F(Args...)>::type>} \DIFaddend that refers
