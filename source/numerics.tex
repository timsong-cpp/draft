...
of \DIFdelbegin \DIFdel{CopyConstructible }\DIFdelend \DIFaddbegin \tcode{CopyConstructible} \DIFaddend (Table~\ref{copyconstructible})
and \DIFdelbegin \DIFdel{CopyAssignable }\DIFdelend \DIFaddbegin \tcode{CopyAssignable} \DIFaddend (Table~\ref{copyassignable}) types.
...
\begin{enumeratea}
  ...
  \item
    ...
    \DIFdelbegin \DIFdel{satisfing }\DIFdelend \DIFaddbegin \DIFadd{satisfying }\DIFaddend the requirements
    of a uniform random number generator [\ref{rand.req.urng}];
  \item
    \tcode{os} is an lvalue of the type of some class template specialization
    \tcode{basic_ostream<charT,} \tcode{traits>};
...
of \DIFdelbegin \DIFdel{CopyConstructible }\DIFdelend \DIFaddbegin \tcode{CopyConstructible} \DIFaddend (Table~\ref{copyconstructible}),
\DIFdelbegin \DIFdel{CopyAssignable }\DIFdelend \DIFaddbegin \tcode{CopyAssignable} \DIFaddend (Table~\ref{copyassignable}),
and
\DIFdelbegin \DIFdel{EqualityComparable }\DIFdelend \DIFaddbegin \tcode{EqualityComparable} \DIFaddend (Table~\ref{equalitycomparable}) types.
...
\pnum
 No function described in this section~\ref{rand.util.seedseq}
 throws an exception.
...
\DIFdelbegin %DIFDELCMD < \indexlibrary{\idxcode{piecewise_constant_distribution}!\idxcode{densitities}}%%%
\DIFdelend \DIFaddbegin \indexlibrary{\idxcode{piecewise_constant_distribution}!\idxcode{densities}}\DIFaddend %
\DIFdelbegin %DIFDELCMD < \indexlibrary{\idxcode{densitities}!\idxcode{piecewise_constant_distribution}}%%%
\DIFdelend \DIFaddbegin \indexlibrary{\idxcode{densities}!\idxcode{piecewise_constant_distribution}}\DIFaddend %
\begin{itemdecl}
vector<result_type> densities() const;
\end{itemdecl}
...
\DIFdelbegin %DIFDELCMD < \indexlibrary{\idxcode{piecewise_linear_distribution}!\idxcode{densitities}}%%%
\DIFdelend \DIFaddbegin \indexlibrary{\idxcode{piecewise_linear_distribution}!\idxcode{densities}}\DIFaddend %
\DIFdelbegin %DIFDELCMD < \indexlibrary{\idxcode{densitities}!\idxcode{piecewise_linear_distribution}}%%%
\DIFdelend \DIFaddbegin \indexlibrary{\idxcode{densities}!\idxcode{piecewise_linear_distribution}}\DIFaddend %
\begin{itemdecl}
vector<result_type> densities() const;
\end{itemdecl}
...
If the length of \tcode{v} is not equal to the length of \tcode{*this}, resizes \tcode{*this} to make the two arrays the same length, as if by calling
\tcode{resize(v.size())}, before performing the assignment.
...
template object which has reference semantics to the \DIFdelbegin \DIFdel{orginal }\DIFdelend \DIFaddbegin \DIFadd{original }\DIFaddend array, working in
...
\pnum
\enternote The overloads provided in C by magic macros are already
...
\tcode{rand}. It is implementation-defined whether the \tcode{rand} function may introduce data races~(\ref{res.on.data.races}). \enternote The random number generation~(\ref{rand}) facilities in this standard are often preferable to \tcode{rand}. \exitnote
...
\begin{enumerate}
  \item If any argument corresponding to a \tcode{double} parameter has type
        \tcode{long double}, then all arguments corresponding to \tcode{double} parameters
        are effectively cast to \tcode{long double}.
  \item Otherwise, if any argument corresponding to a \tcode{double} parameter has
        type \tcode{double} or an integer type, then all arguments corresponding to
        \tcode{double} parameters are effectively cast to \tcode{double}.
  \item Otherwise, all arguments corresponding to \tcode{double} parameters are
        effectively cast to \tcode{float}.
\end{enumerate}
