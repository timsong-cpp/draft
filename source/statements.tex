...
\DIFdelbegin %DIFDELCMD < \indextext{statment!range~based \idxcode{for}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \indextext{statement!range~based \idxcode{for}}
\DIFaddend 
...
up with argument-dependent lookup~(\ref{basic.lookup.argdep}). For the purposes of this
name lookup, namespace \tcode{std} is an associated namespace.
\end{itemize}
