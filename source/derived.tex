...
\grammarterm{using-declaration}{s} are replaced by the members they
...
struct D : B {
  \DIFaddbegin \DIFadd{virtual }\DIFaddend void f(long) override;  // error: wrong signature overriding \tcode{B::f}
  \DIFaddbegin \DIFadd{virtual }\DIFaddend void f(int) override;   // OK
};
\end{codeblock}
\exitexample
