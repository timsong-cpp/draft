...
potentially-evaluated expression is \defn{odr-used} unless it is an
object that satisfies the requirements for appearing in a
constant expression~(\ref{expr.const}) and the lvalue-to-rvalue
conversion~(\ref{conv.lval}) is immediately applied.
...
initialization~(\ref{dcl.init}). A copy constructor or move constructor is odr-used even if the
...
move-assignment function for another class as specified in~\ref{class.copy}.
...
in~\ref{dcl.init}. A destructor for a class is odr-used as specified
in~\ref{class.dtor}.
...
\rSec2[basic.scope.local]{Block scope}
...
\grammarterm{brace-or-equal-initializers} of non-static data members, and
default arguments
...
\grammarterm{parameter-declaration-clause} or in the function body.}
or in the \grammarterm{brace-or-equal-initializer} of a non-static
data member~(\ref{class.mem}) of class \tcode{X}
shall be declared in one of the following ways:
...
the \grammarterm{nested-name-specifier} nominates a class \tcode{C}:
...
noted, a name declared at block scope~(\ref{basic.scope.local}) has no
...
\item it is a specialization of a class template~(\ref{temp})\footnote{A class
...
type \tcode{std::type_info}~(\ref{expr.typeid}), or for the copy of an
...
of which has type \tcode{std::size_t}~(\ref{support.types}), then this
...
or
\item a sequence of elements in an array of character type, where the size and alignment
...
on dereferencing pointers to memory not allocated by \tcode{::operator new}. This
...
because dereferencing pointers to memory allocated by \tcode{malloc} is not restricted.}

\item the result of taking the address of an
...
safety}, in which case a pointer value that is not a safely-derived pointer
...
\tcode{char*}, or \tcode{unsigned} \tcode{char*}), or
...
\item the glvalue is implicitly converted~(\ref{conv.ptr}) to a reference
to a base class type, or

\item the glvalue is used as the operand of a
\tcode{static_cast}~(\ref{expr.static.cast}) except when the conversion
is ultimately to \term{cv} \tcode{char\&} or \term{cv}
\tcode{unsigned} \tcode{char\&}, or
...
\item a reference type \DIFaddbegin \DIFadd{referring to a literal type}\DIFaddend ; or
\item \DIFaddbegin \DIFadd{an array of literal type; or
}\item \DIFaddend a class type (Clause~\ref{class}) that
...
\item \DIFdelbegin \DIFdel{it has all }\DIFdelend \DIFaddbegin \DIFadd{all of its }\DIFaddend non-static data members and base classes \DIFaddbegin \DIFadd{are
}\DIFaddend of literal types\DIFdelbegin \DIFdel{; or
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend \end{itemize}
\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdel{an array of literal type.
...
unsigned integer types are collectively called the \term{extended integer types}.
...
respectively, in \tcode{<stdint.h>}, called the underlying types.
...
\tcode{typeid} or \tcode{decltype}, as
...
the types \tcode{char}, \tcode{signed char}, and
\tcode{unsigned char} shall have the weakest alignment requirement.
\enternote This enables the character types to be used as the
