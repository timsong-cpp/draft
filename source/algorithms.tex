...
The type of
\tcode{*first}
shall satisfy the \DIFdelbegin \DIFdel{MoveAssignable
}\DIFdelend \DIFaddbegin \tcode{MoveAssignable}
\DIFaddend requirements (Table~\ref{moveassignable}).
...
and
\range{result}{result + (last - first)}
shall not overlap. The expression \tcode{*result = *first} shall ve valid.
...
\tcode{InputIterator}
shall be \DIFdelbegin \DIFdel{CopyConstructible }\DIFdelend \DIFaddbegin \tcode{CopyConstructible} \DIFaddend (Table~\ref{copyconstructible}) and
\DIFdelbegin \DIFdel{CopyAssignable }\DIFdelend \DIFaddbegin \tcode{CopyAssignable} \DIFaddend (Table~\ref{copyassignable}).
Otherwise \DIFdelbegin \DIFdel{CopyConstructible }\DIFdelend \DIFaddbegin \tcode{CopyConstructible} \DIFaddend is not required.
...
\DIFdelbegin \DIFdel{ValueSwappable}\DIFdelend \DIFaddbegin \tcode{ValueSwappable}\DIFaddend ~(\ref{swappable.requirements}). The type of \tcode{*first} shall satisfy
...
generating function object \tcode{rand} shall have a return type that is
convertible to \tcode{iterator_traits<RandomAccessItera\-tor>::difference_type},
and the call \tcode{rand(n)} shall return a randomly chosen value in the
interval \range{0}{n}, for \tcode{n > 0} of type
\tcode{iterator_traits<Random\-Access\-Itera\-tor>::difference_type}. The type
...
In the second form of the function, the
 function object
\tcode{rand} shall serve as the implementation's source of randomness.

In the third \tcode{shuffle} form of the function, the object \tcode{g} shall serve as the implementation's source of randomness.
...
\begin{itemdescr}
\pnum
\requires \tcode{InputIterator}'s value type shall be Assignable, and shall be writable to the \tcode{out_true} and \tcode{out_false} \tcode{OutputIterator}s, and shall be convertible to \tcode{Predicate}'s argument type. The input range shall not overlap with either of the output ranges.
...
an object of type \tcode{Compare}, when contextually converted to \tcode{bool}~(\ref{conv}),
