...
\term{single pass}
algorithms.
Value type T is not required to be a \DIFdelbegin \DIFdel{CopyAssignable }\DIFdelend \DIFaddbegin \tcode{CopyAssignable} \DIFaddend type (Table~\ref{copyassignable}).
...
\DIFdelbegin \DIFdel{.
is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend referenced in a way that requires instantiation~(\ref{temp.inst}).
