...
Clauses~\ref{language.support} through~\ref{thread} and Annex~\ref{depr})
...
\tcode{a.deallocate(p,n)}   &
  ...
  to \tcode{allocate} which has not been invalidated by
...
\tcode{a.max_size()}        &
  ...
  \tcode{numeric_limits<size_type>::max()/sizeof(value_type)}  \\ \rowsep
