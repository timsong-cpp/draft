...
\tcode{<list>} &
\tcode{<regex>} &
\DIFdelbegin %DIFDELCMD < \tcode{<typeindex>}         %%%
\DIFdelend \DIFaddbegin \tcode{<tuple>} \DIFaddend \\

\tcode{<array>} &
\tcode{<functional>} &
\tcode{<locale>} &
\DIFdelbegin %DIFDELCMD < \tcode{<set>}               %%%
\DIFdelend \DIFaddbegin \tcode{<scoped_allocator>} \DIFaddend &
\DIFdelbegin %DIFDELCMD < \tcode{<typeinfo>}          %%%
\DIFdelend \DIFaddbegin \tcode{<type_traits>} \DIFaddend \\

\tcode{<atomic>} &
\tcode{<future>} &
\tcode{<map>} &
\DIFdelbegin %DIFDELCMD < \tcode{<sstream>}           %%%
\DIFdelend \DIFaddbegin \tcode{<set>} \DIFaddend &
\DIFdelbegin %DIFDELCMD < \tcode{<type_traits>}       %%%
\DIFdelend \DIFaddbegin \tcode{<typeindex>} \DIFaddend \\

\tcode{<bitset>} &
\tcode{<initializer_list>} &
\tcode{<memory>} &
\DIFdelbegin %DIFDELCMD < \tcode{<stack>}             %%%
\DIFdelend \DIFaddbegin \tcode{<sstream>} \DIFaddend &
\DIFdelbegin %DIFDELCMD < \tcode{<unordered_map>}     %%%
\DIFdelend \DIFaddbegin \tcode{<typeinfo>} \DIFaddend \\

\tcode{<chrono>} &
\tcode{<iomanip>} &
\tcode{<mutex>} &
\DIFdelbegin %DIFDELCMD < \tcode{<stdexcept>}         %%%
\DIFdelend \DIFaddbegin \tcode{<stack>} \DIFaddend &
\DIFdelbegin %DIFDELCMD < \tcode{<unordered_set>}     %%%
\DIFdelend \DIFaddbegin \tcode{<unordered_map>} \DIFaddend \\

\tcode{<codecvt>} &
\tcode{<ios>} &
\tcode{<new>} &
\DIFdelbegin %DIFDELCMD < \tcode{<streambuf>}         %%%
\DIFdelend \DIFaddbegin \tcode{<stdexcept>} \DIFaddend &
\DIFdelbegin %DIFDELCMD < \tcode{<utility>}           %%%
\DIFdelend \DIFaddbegin \tcode{<unordered_set>} \DIFaddend \\

\tcode{<complex>} &
\tcode{<iosfwd>} &
\tcode{<numeric>} &
\DIFdelbegin %DIFDELCMD < \tcode{<string>}            %%%
\DIFdelend \DIFaddbegin \tcode{<streambuf>} \DIFaddend &
\DIFdelbegin %DIFDELCMD < \tcode{<valarray>}          %%%
\DIFdelend \DIFaddbegin \tcode{<utility>} \DIFaddend \\

\tcode{<condition_variable>} &
\tcode{<iostream>} &
\tcode{<ostream>} &
\DIFdelbegin %DIFDELCMD < \tcode{<strstream>}         %%%
\DIFdelend \DIFaddbegin \tcode{<string>} \DIFaddend &
\DIFdelbegin %DIFDELCMD < \tcode{<vector>}            %%%
\DIFdelend \DIFaddbegin \tcode{<valarray>} \DIFaddend \\

\tcode{<deque>} &
\tcode{<istream>} &
\tcode{<queue>} &
\DIFdelbegin %DIFDELCMD < \tcode{<system_error>}      %%%
\DIFdelend \DIFaddbegin \tcode{<strstream>} \DIFaddend &
\DIFaddbegin \tcode{<vector>} \DIFaddend \\

\tcode{<exception>} &
\tcode{<iterator>} &
\tcode{<random>} &
\DIFdelbegin %DIFDELCMD < \tcode{<thread>}            %%%
\DIFdelend \DIFaddbegin \tcode{<system_error>} \DIFaddend & \\

\tcode{<forward_list>} &
\tcode{<limits>} &
\tcode{<ratio>} &
\DIFdelbegin %DIFDELCMD < \tcode{<tuple>}             %%%
\DIFdelend \DIFaddbegin \tcode{<thread>} \DIFaddend & \\
...
before the first reference in that translation unit to any of the entities declared in that header.
...
\DIFdelbegin %DIFDELCMD < \begin{concepttable}{\tcode{CopyConstructible} requirements (in addition to MoveConstructible)}{copyconstructible}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{concepttable}{\tcode{CopyConstructible} requirements (in addition to \tcode{MoveConstructible})}{copyconstructible}
\DIFaddend {p{1in}p{4.15in}}
\topline
Expression          &   Post-condition  \\ \capsep
...
\DIFdelbegin %DIFDELCMD < \begin{concepttable}{\tcode{CopyAssignable} requirements(in addition to MoveAssignable)}{copyassignable}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{concepttable}{\tcode{CopyAssignable} requirements (in addition to \tcode{MoveAssignable})}{copyassignable}
\DIFaddend {p{1in}p{1in}p{1in}p{1.9in}}
\topline
Expression  &   Return type &   Return value    &   Post-condition  \\ \capsep
...
\DIFdelbegin %DIFDELCMD < \rSec3[swappable.requirements]{Swappable requirements}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \rSec3[swappable.requirements]{\tcode{Swappable} requirements}
\DIFaddend 
...
\DIFdelbegin \DIFdel{is }%DIFDELCMD < \defn{ValueSwappable} %%%
\DIFdelend \DIFaddbegin \DIFadd{satisfies the requirements of }\tcode{ValueSwappable} \DIFaddend if,
...
namespace N {
  ...
  void swap(Proxy p, A& x) { swap(x, p); }      // satisfy symmetry \DIFdelbegin \DIFdel{contraint
}\DIFdelend \DIFaddbegin \DIFadd{constraint
}\DIFaddend }
...
\item it satisfies the requirements of \tcode{CopyConstructible} and
  \tcode{Destructible}~(\ref{utility.arg.requirements}), \DIFaddbegin \DIFadd{and
}\DIFaddend \item the expressions shown in Table~\ref{hash}
are valid and have the indicated semantics\DIFdelbegin \DIFdel{, and
}%DIFDELCMD < \item %%%
\DIFdel{it satisfies all other requirements in this subclause}\DIFdelend .
\end{itemize}
...
\tcode{h(k)}      &
  ...
  \tcode{k}. \enternote Thus all evaluations of the expression \tcode{h(k)} with the
  same value for \tcode{k} yield the same result. \exitnote \enternote For two different
...
\lhdr{Variable} &   \rhdr{Definition}   \\  \capsep
\endhead
\tcode{T, U, C}    &   any non-const \DIFdelbegin \DIFdel{, non-reference object type}\DIFdelend \DIFaddbegin \DIFadd{object type~(\ref{basic.types})       }\DIFaddend \\ \rowsep
...
\tcode{c}       &   a dereferenceable pointer of type \tcode{C*}  \\ \rowsep
...
\tcode{u}       &   a value of type \tcode{YY::const_pointer}
obtained by calling \tcode{YY::allocate}, or else \tcode{nullptr}. \\ \rowsep
...
\pnum
\DIFdelbegin %DIFDELCMD < \indextext{definition!altermate}%%%
\DIFdelend \DIFaddbegin \indextext{definition!alternate}\DIFaddend %
Clauses~\ref{\firstlibchapter} through~\ref{\lastlibchapter} and Annex~\ref{depr}
...
\item For the \term{remove} algorithms the relative order of
the elements that are not removed is preserved.

\item For the \term{merge} algorithms, for equivalent elements in
the original two ranges, the elements from the first range precede
...
\DIFdelbegin %DIFDELCMD < \impldef{error_category@\tcode{error_category} for errors originating outside the
%DIFDELCMD < operationg system} %%%
\DIFdelend \DIFaddbegin \impldef{error_category@\tcode{error_category} for errors originating outside the
operating system} \DIFaddend \tcode{error_category} object for errors originating elsewhere.
